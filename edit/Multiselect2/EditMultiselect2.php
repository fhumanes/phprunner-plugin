...
class EditMultiselect2 extends UserControl
{
	...
	function initUserControl()
	{
		...
		if($this->settings["selectableHeader"]) $this->selectableHeader=$this->settings["selectableHeader"];
		if($this->settings["selectionHeader"]) $this->selectionHeader=$this->settings["selectionHeader"];
		if($this->settings["selectableFooter"]) $this->selectableFooter=$this->settings["selectableFooter"];
		if($this->settings["selectionFooter"]) $this->selectionFooter=$this->settings["selectionFooter"];
        if($this->settings["height"]) $this->height=$this->settings["height"];
        if($this->settings["width"]) $this->width=$this->settings["width"];
		if($this->settings["selectableOptgroup"]) $this->selectableOptgroup=$this->settings["selectableOptgroup"];			
		if($this->settings["selectDeselectAll"]) $this->selectDeselectAll=$this->settings["selectDeselectAll"];
		if($this->settings["table"] && $this->settings["table"] != '') $this->table=$this->settings["table"];
		if($this->settings["link_field"] && $this->settings["link_field"] != '') $this->link_field=$this->settings["link_field"];		
		if($this->settings["display_field"] && $this->settings["display_field"] != '') $this->display_field=$this->settings["display_field"];	
		if($this->settings["order_by"] && $this->settings["order_by"] != '') $this->order_by=$this->settings["order_by"];	
		if($this->settings["where"] && $this->settings["where"] != '') $this->where = $this->settings["where"];		
...
EOT;
	...
	function addJSFiles()
	{
		$this->pageObject->AddJSFile("plugins/controles/Multiselect2/jquery.multi-select.js");
	}	

	function addCSSFiles()
	{
		$this->pageObject->AddCSSFile("plugins/controles/Multiselect2/css/multi-select.css");
	}		
}
